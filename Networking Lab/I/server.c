...
int main(int argc, char const *argv[])
{
    ...
    valread=recvfrom(server_fd,&buf,sizeof(int),MSG_WAITALL,(struct sockaddr*)&cliaddr1,&len1);
    printf("Number Received :%d\n",buf);
    
    int sq=buf*buf;
    recvfrom(server_fd,0,0,MSG_WAITALL,(struct sockaddr*)&cliaddr2,&len2);
    sendto(server_fd,&sq,sizeof(int),MSG_CONFIRM,(struct sockaddr*)&cliaddr2,len2);
